...
- [x] **OBS-06**: Failed webhook events (WhatsApp, Bokun, Stripe) are persisted to Convex with error details; admin dashboard provides a simple view and manual retry capability
...
- [x] **TEST-01**: Automated tests cover the booking state machine happy path: select_time → select_pickup → ask_participants → confirm
- [x] **TEST-02**: Automated tests cover the cancellation flow triggered mid-booking
...
| OBS-06b (admin UI + retry) | Phase 4 | Complete |
