...
export async function handleListPickupPlaces(
  ...
  if (!draft?.activityId || !draft.date) {
    return {
      text: "Antes de escolher pickup, preciso que você selecione atividade, data e horário.",
      pickupOptionMap: [],
    };
  }
  ...
  try {
    ...
    return {
      text: "Erro ao consultar locais de pickup. Tente novamente em alguns instantes.",
      pickupOptionMap: [],
    };
  }
